...
@section('title', 'Laporan Kehadiran Pelamar')
@section('page_title', 'Laporan Kehadiran Pelamar')

@section('content')
<div class="card shadow-sm border-0 mb-8">
    <div class="card-body">
        <form action="{{ route('admin.laporan.kehadiran') }}" method="GET" class="row g-5 align-items-end">
            <div class="col-md-2">
            ...
            <table class="table table-bordered align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="fw-bold fs-6 text-gray-800">{{ $lamaran->pelamar->nama }}</div>
                            <div class="text-muted fs-8">{{ $lamaran->pelamar->email }}</div>
                        </td>
                        <td>
                            <div class="fw-bold fs-7">{{ $lamaran->lowongan->namalowongan }}</div>
                            <div class="text-muted fs-8">{{ $lamaran->lowongan->register->perusahaan->nama ?? 'N/A' }}</div>
                        </td>
                        <td class="text-center fs-7">{{ $lamaran->sesi->nama_sesi ?? 'Reguler' }}</td>
                        <td class="text-center">
                            @if($hadir)
                                <span class="badge badge-success fw-bold">HADIR</span>
                            @else
                                <span class="badge badge-light-danger fw-bold">TIDAK HADIR</span>
                            @endif
                        </td>
                        <td class="text-center">
                            @if($hadir)
                                <span class="fw-bold fs-7">{{ \Carbon\Carbon::parse($lamaran->kehadirans->first()->tanggal)->format('d-m-Y') }}</span>
                                <div class="text-gray-500 fs-9">{{ $lamaran->kehadirans->first()->jam }}</div>
                            @else
                                -
                            @endif
                        </td>
