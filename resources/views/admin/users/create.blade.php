...
<div class="card shadow-sm">
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            <h3 class="fw-bold">Form Pengguna Baru</h3>
        </div>
    ...
    <div class="card-body">
        <form id="kt_user_create_form" action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            
            <div class="row mb-6">
                <label class="col-lg-4 col-form-label required fw-bold fs-6">Nama Lengkap</label>
                <div class="col-lg-8 fv-row">
                    <input type="text" name="name" class="form-control form-control-lg form-control-solid" placeholder="Masukkan nama lengkap" value="{{ old('name') }}" required />
                </div>
            </div>
            ...
            <div class="row mb-6">
                ...
                <div class="col-lg-8 fv-row">
                    <input type="email" name="email" class="form-control form-control-lg form-control-solid" placeholder="email@example.com" value="{{ old('email') }}" required />
                </div>
            </div>
                ...
                <div class="col-lg-8 fv-row">
                    <select name="ideven" class="form-select form-select-lg form-select-solid" data-control="select2" data-placeholder="Pilih Event Bursa Kerja...">
                        ...
                        @foreach($events as $even)
                            <option value="{{ $even->id }}" {{ old('ideven') == $even->id ? 'selected' : '' }}>{{ $even->namaperiode }}</option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-6">
                ...
                <div class="col-lg-8 fv-row">
                    <input type="text" class="form-control form-control-lg form-control-solid" value="password" readonly />
                    <div class="form-text text-muted">Password otomatis diset ke <strong>password</strong>. Pengguna dapat mengubahnya nanti.</div>
                </div>
            </div>
            ...
            <div class="row mb-6">
                ...
                <div class="col-lg-8">
                    ...
                    <div class="form-text">Tipe file yang diizinkan: png, jpg, jpeg. Mbksimum 2MB.</div>
                </div>
            </div>

            <div class="d-flex justify-content-end">
                <button type="reset" class="btn btn-light me-3">Batal</button>
                <button type="submit" id="kt_user_create_submit" class="btn btn-primary d-flex align-items-center">
                    <span class="indicator-label d-flex align-items-center">
                        <i class="material-icons me-2">save</i> Simpan Pengguna
                    </span>
...
$(document).ready(function() {
    const form = document.querySelector('#kt_user_create_form');
    const submitButton = document.querySelector('#kt_user_create_submit');
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                if (xhr.status === 422) {
                    ...
                    message = Object.values(errors)[0][0]; // Ambil error pertama
                    
                    // Highlight fields based on errors if needed
                    // (Optional) but PNotify is enough for now as requested
